...
class SignUpForm(UserCreationForm):
	mobile_no = forms.CharField(max_length=10, required=True, widget=forms.TextInput(attrs={'class':'form-control'}))
	user_type = forms.ChoiceField(choices=CHOICES, widget=forms.RadioSelect())
	password1 = forms.CharField(label=("Enter Pssword"), widget=forms.PasswordInput(attrs={'class':'form-control'}))
...
class PropertyRent(forms.ModelForm):
	lease_type = forms.ChoiceField(choices=[('f','Family'),('a','Anyone'),('c','Company')], widget=forms.Select(attrs={'class':'js-example-basic-single js-states form-control'}))
	furnished = forms.ChoiceField(choices=[('0','Not Furnished'),('1','Fully Furnished')], widget=forms.RadioSelect())
	rooms = forms.ChoiceField(choices=[('1','1BHK'),('2','2BHK'),('3','3BHK'),('4','4BHK')], widget=forms.Select(attrs={'class':'js-example-basic-single js-states form-control'}))
	class Meta:
		model = Properties
